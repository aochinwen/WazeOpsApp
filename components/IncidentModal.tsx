...
import { X, MapPin, Share2, Copy, ExternalLink, CheckCircle, Clock, AlertTriangle } from 'lucide-react';
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="p-6 overflow-y-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-3">
             <button 
                onClick={() => onUpdateStatus(incident.uuid, IncidentStatus.ACKNOWLEDGED)}
                className={`flex items-center justify-center gap-2 p-3 rounded-lg border font-medium transition-all ${incident.status === IncidentStatus.ACKNOWLEDGED ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
             >
               <Clock size={18} />
               Acknowledge
             </button>
             <button 
                onClick={() => onUpdateStatus(incident.uuid, IncidentStatus.RESOLVED)}
                className={`flex items-center justify-center gap-2 p-3 rounded-lg border font-medium transition-all ${incident.status === IncidentStatus.RESOLVED ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 border-gray-200 hover:bg-gray-50'}`}
             >
               <CheckCircle size={18} />
               Resolve
             </button>
