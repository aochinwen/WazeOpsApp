...
const FEED_SOURCES = [
    ...
    { id: 'NSC-N113', name: 'NSC-113', url: 'https://www.waze.com/row-partnerhub-api/partners/18727209890/waze-feeds/a2141ab7-ad83-4456-a614-54c572a780d4?format=1' },
    ...
    if (client_id && client_secret && project_id) {
        try {
            ...
            try {
                ...
                if (cctvUrlSecret === null || cctvUrlSecret === void 0 ? void 0 : cctvUrlSecret.secretValue)
                    CCTV_URL = cctvUrlSecret.secretValue;
            }
            catch (_a) {
...
app.post('/collect', async (req, res) => {
    ...
    try {
        await axios_1.default.post(`https://www.google-analytics.com/mp/collect?measurement_id=${GA_MEASUREMENT_ID}&api_secret=${GA_API_SECRET}`, { client_id, events }, { headers: { 'Content-Type': 'application/json' } });
...
async function loadAllSecrets() {
    ...
    if (client_id && client_secret && project_id) {
        try {
            ...
            if (geminiSecret)
                GEMINI_API_KEY = geminiSecret.secretValue;
            console.log("Gemini Key Loaded from Infisical");
        }
        catch (e) {
            console.error("Gemini Key Load Failed", e);
        }
    }
}
app.post('/summarize', async (req, res) => {
    ...
    const isValidIncident = incidents.every((i) => i && typeof i === 'object' &&
        (i.type || i.subtype) &&
        (i.street || i.city));
    if (!isValidIncident) {
